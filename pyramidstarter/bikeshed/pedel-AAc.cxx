...
distr=1;
  ...
  for (i = 0; i < nlines - 1; ++i) {
    ...
    if (0 == i) { strcpy(inputseq, seqline); } 
    else { strcat(inputseq, seqline); }
  }
  sequence1.close();
  for (i = 0; ; ++i) {
  ...
  */
  ...
  libtable << "<table cellspacing=\"0\" cellpadding=\"3\" border=\"1\">\n";
  libtable << "<tr><th align=\"center\" colspan=\"1\"></th>\n"
	   << "<th align=\"center\" colspan=\"2\">"
	   << "<a href=/aef/STATS/FORM/pedel-AA_variants.html>variants</a>"
	   << "</th>\n<th align=\"center\" colspan=\"5\">"
	   << "<a href=/aef/STATS/FORM/pedel-AA_poisson.html>Poisson</a>"
	   << "</th>\n"
	   << "<th align=\"center\" colspan=\"5\">"
	   << "<a href=/aef/STATS/FORM/pedel-AA_pcr.html>PCR</a>"
	   << "</th></tr>\n";
  ...
  for (i = 0; i <= maxx; ++i) {
    ...
    if (0 == distr) {
      if (i > 2) {
	if (-1 == CxExactPCR[i]) {
	  libtable << RxPCR[i] << "</td><td>"
		   << LxPCR[i] << "</td><td>" << CxCombPCR[i] << "</td><td>"
		   << "<a href=/aef/STATS/FORM/pedel-AA_warning.html>warning"
		   << "</a></td><td>" << LxCxCombPCR[i] << "</td></tr>\n";
	PCRwarning = 1;
	} else {
	  if (RxPCR[i] < 0.1) {
	    libtable << RxPCR[i] << "</td><td>"
		     << LxPCR[i] << "</td><td>" << CxCombPCR[i] << "</td><td>" 
		     << "<a href=/aef/STATS/FORM/pedel-AA_CxLx.html>Cx ~ Lx" 
		     << "</a></td><td>" << LxCxCombPCR[i] << "</td></tr>\n";
	  } else {
	    libtable << RxPCR[i] << "</td><td>"
		     << LxPCR[i] << "</td><td>" << CxCombPCR[i] << "</td><td>" 
		     << "<a href=/aef/STATS/FORM/pedel-AA_warningRx.html>"
		     << "Rx warning</a></td><td>" << LxCxCombPCR[i] 
		     << "</td></tr>\n";
	  }
	}
      } else {
	libtable << RxPCR[i] << "</td><td>"
		 << LxPCR[i] << "</td><td>" << CxCombPCR[i] << "</td><td>" 
		 << "<a href=/aef/STATS/FORM/pedel-AA_exact.html>Exact</a>" 
		 << "</td><td>" << LxCxCombPCR[i] << "</td></tr>\n";
      }
    } else {
      libtable << "-</td><td>-</td><td>-</td><td>-</td><td>-</td></tr>\n";
    }
  }
