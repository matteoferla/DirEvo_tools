...
int main(int argc, char * argv[]) {
    ...
    if (4 != argc) {
        cout << "Usage '" << argv[0] << " library_size sequence_length " << "mean_number_of_mutations_per_sequence'.\n";
        exit(EXIT_FAILURE);
    }
    ...
    if (L < 0.) {
        cout << "Error: Library size can't be negative.  " << "You entered " << L << ".<br>\n";
        exit(EXIT_FAILURE);
    }
    if (L < 10.) {
        cout << "Warning: Library size very small.  " << "Statistics may be compromised.<br>\n";
    }
    if (N < 1.) {
        cout << "Error: Sequence length must be positive integer.  " << "You entered " << N << ".<br>\n";
        exit(EXIT_FAILURE);
    }
    if (N < 5.) {
        cout << "Warning: Sequence length very small.  " << "Statistics may be compromised.<br>\n";
    }
    if (lambda < 0.) {
        cout << "Error: Mean number of mutations can't be negative.  " << "You entered " << lambda << ".<br>\n";
        exit(EXIT_FAILURE);
    }
    if (lambda > 0.1 * N) {
        cout << "Warning: Mutation rate is high.  " << "Statistics may be compromised.<br>\n";
    }
    if (lambda > 0.75 * N) {
        cout << "Error: Maximum mutation rate is 0.75 * sequence length = " << 0.75 * N << ".  " << "You entered " << lambda << ".<br>\n";
        exit(EXIT_FAILURE);
    }
    ...
    cout << 0 << " " << exp(-lambda) << " " << L * exp(-lambda) << " " << 1. << " " << cx0 << " " << cx0 << " " << L * exp(-lambda) - cx0 << "\n";
    xfac = 1.;
    nfac = 1.;
    maxx = 20;
    if (maxx > int(N)) {
    ...
    for (i = 1; i <= maxx; ++i) {
        ...
        cout << i << " " << Px << " " << Lx << " " << Vx << " " << Cx << " " << Cx / Vx << " " << Lx - Cx << "\n";
    }
    outfile.close();
