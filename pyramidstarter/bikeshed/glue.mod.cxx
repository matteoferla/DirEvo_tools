...
int main(int argc, char* argv[])
{
    ...
    if (1 == mode) {
        L = atof(argv[3]);
        if (L <= 0) {L = 0;}
        cout << "number_of_possible_variants = " << var << ".<br>\n";
        cout << "library_size  = " << L << ".<br>\n";
        cout << "completeness = " << 1.-exp(-L/var) << ".<br>\n";
        ...
        if(-log(p100)/var >= 0.1) {
            cerr << "Warning: Probability too small to continue.<br>\n";
            exit(EXIT_FAILURE);
        }
        cout << "Number of possible variants =  " << var << ".<br>\n";
        cout << "prob_complete = " << p100
        << ".<br>\n";
        cout << "Required library size = " << -var*log(-log(p100)/var) << ".<br>\n";

    } else {
        cerr << "Unsupported running mode: " << mode << ".<br>\n";
        exit(EXIT_FAILURE);
    }
