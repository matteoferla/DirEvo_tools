...
class Fields:
    ...
    terms = {
        ...
        'bias': ('Mutational bias',
                 'Various metrics to assess the divergence from the ideal situation where a nucleotide position has equiprobably change of mutating into any of the other three.'),
        'rate': ('error rate per division',
                 'The average number of de novo mutations per kb that happen during one PCR duplication cycle'),
        'transversion': ('Transversions',
                         'This is a mutation where a purine (adenine, guanine),  becomes a pyrimidine (thymine/uracil, cytosine) or vice versa.'),
        'transition': (
        ...
        'missense': ('Missense mutation',
                     'A mutation that results in an amino acid change. These are good mutations for directed evolution.'),
        'nonsense': ('Nonsense mutation',
                     'A mutation that results in a premature stop. These are bad mutations for directed evolution.'),
        'synonymous': ('Synonymous mutation',
                       'A mutation that alters the codon into another that codes the same amino acid. one third of changes in a equiprobable scenario are synonymous.'),
        'epPCR': ('Error-prone PCR',
                  'A PCR reaction with a polymerase without proof-reading ability that may have lower fidelity due to amino acid changes (Mutazyme) or cofactor (manganese) or in the presence of wobbly dNTPs (oxodGTP and dPTP). This differs from PCR where the primers carry the variation (QuickChange-like).')
    }

    def term_helper(self, term, inner=None):
        if inner:
            return '<abbr data-toggle="tooltip" title="{t}">{i}</abbr>'.format(i=inner, t=self.terms[term][0] + ': ' +
                                                                                          self.terms[term][1])
        else:
            return '<abbr data-toggle="tooltip" title="{t}">{i}</abbr>'.format(i=term, t=self.terms[term][0] + ': ' +
                                                                                         self.terms[term][1])
